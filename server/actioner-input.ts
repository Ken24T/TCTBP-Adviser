...
export async function readActionerRequest(
  ...
  if (
    !['checkpoint', 'publish', 'deploy-development', 'branch-development'].includes(String(value.workflowId))
  ...
  return {
    workflowId: value.workflowId as 'checkpoint' | 'publish' | 'deploy-development' | 'branch-development',
